...
const{uid,now,getRiskScore}=require('../utils/helpers');
const{PLANS}=require('../services/planLimits');

async function logAndFlag(agentId,action,authorizationId,status,reason,agent,auth,metadata,orgId){
  try{
    ...
    const incidentReasons=['REPLAY_ATTEMPT','CROSS_ORG_ATTEMPT','AGENT_MISMATCH'];
    if(incidentReasons.includes(reason)){
      await db.insertIncident({id:uid('inc_'),org_id:orgId,type:reason,detail:{agentId,action,authorizationId},ai_summary:'Security event: '+reason+' detected for agent '+(agent?.name||agentId)+' attempting "'+action+'".'});
    }
  } catch(err){console.error('[logAndFlag]',err.message);}
}
...
router.post('/',requireApiKey,async(req,res)=>{
  ...
  if(monthlyCount>=plan.maxExecutions){
    ...
    return res.status(429).json({error:'Monthly execution limit reached',status:'denied',reason:'PLAN_LIMIT_EXCEEDED',plan:orgPlan,limit:plan.maxExecutions,used:monthlyCount,upgradeUrl:'https://agentpassport.in/pricing'});
  }

  if(agent.status!=='active'){
    await logAndFlag(agentId,action,authorizationId,'denied','AGENT_SUSPENDED',agent,null,metadata,req.org.id);
    return res.status(403).json({error:'Agent is suspended',status:'denied',reason:'AGENT_SUSPENDED'});
  }
  ...
  const riskScore=getRiskScore(action);
  ...
  setImmediate(async()=>{
    try{
      ...
      const analysis=await gemini.analyzeExecution({action,status:'success',reason:'VALID',riskScore,agentName:agent.name,agentRole:agent.role,approvedBy:auth.approved_by,metadata});
      if(analysis) await db.updateExecutionAnalysis(execRecord.id,analysis);
      if(plan.incidentDetection){
        const recent=await db.getRecentExecutionsByAgent(agentId,20);
        const incidents=await gemini.detectIncidents({agentName:agent.name,agentRole:agent.role,recentExecutions:recent});
        for(const inc of incidents){
          await db.insertIncident({id:uid('inc_'),org_id:req.org.id,type:inc.type,detail:{agentId,agentName:agent.name,severity:inc.severity},ai_summary:inc.summary});
        }
      }
    } catch(err){console.error('[Execute/Async]',err.message);}
  });
});
